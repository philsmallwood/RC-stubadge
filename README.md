...
The Student Badge LDAP Updater script is designed to update the LDAP information in the MySQL database for the Student Badge application used by students.redclayschools.com. The script imports necessary modules, sets variables including authentication keys and server information, establishes a connection to a remote MySQL Database to retrieve student information, updates the apps MySQL database with the retrieved information, logs the process, writes the log to a Google Drive folder, and sends an email alert in case of errors.

Main Steps:
1. Import necessary modules and set variables.
2. Establish a connection to the remote MySQL Database and retrieve student information.
